...
if exist('varargin','var')
    ...
    for ni = 1:2:L
        switch lower(varargin{ni})
            ...
            case 'ringradii', ringRadii = varargin{ni+1};
        end
    end
end
%% create gui objects
...
%     [.1,.8,.18,.18],'Value',2,'Callback',@setColormap); %#ok<NASGU>
...
hImg = imagesc(hAx.center, log10(abs(img))); %#ok<NASGU>
hAx.center.XLim = [center(2)-128,center(2)+127];
hAx.center.YLim = [center(1)-128,center(1)+127];
hAx.center.Title.String = sprintf('center = [%.1f, %.1f]', center(1), center(2));
hAx.center.CLim(1) = -1;
hold(hAx.center, 'on');
...
wedge(nWedges).radialProfilePlt=[];
...
for i=1:nIterations
    ...
    angleMatrix = atan2(yy - center(1), xx - center(2));
    for w=1:nWedges
        wedge(w).lims = wedgeAngles([w,w+1]);
        wedgeMask = angleMatrix > wedgeAngles(w) & ...
            angleMatrix <= wedgeAngles(w+1);
...
%         wedge(w).rprof = conv(wedge(w).rprof(xData),...
%             ones(movMeanPixel,1), 'valid');
%         wedge(w).rprof = smooth(wedge(w).rprof(rMin:rMax), ...
%             movMeanPixel, 'rloess');
    ...
    for w=1:(nWedges/2)
        ...
        wedge(w).xcorr = wedge(w).xcorr/sum(wedge(w).xcorr);
        [~,maxInd] = max(wedge(w).xcorr);
%         maxInd = sum( wedge(w).xcorr .* (1:numel(wedge(w).xcorr)) );
        
        wedge(w).center_diff = (numel(wedge(w).rprof) - maxInd)/2;
        
        shift = [ wedge(w).center_diff * cos(mean(wedge(w).lims)), ...
            wedge(w).center_diff * sin(mean(wedge(w).lims)) ];
...
% % % % % % % % %     end
    ...
    if i==nIterations || displayFlag
        hAx.center.Title.String = sprintf('center = [%.1f, %.1f]', ...
            center(1), center(2));
        ...
        for w=1:nWedges/2
            ...
            if ~isempty(wedge(w).line)
                wedge(w).line.XData=r*cos(wedge(w).lims(2))+centerOld(2);
                wedge(w).line.YData=r*sin(wedge(w).lims(2))+centerOld(1);
                wedge(w2).line.XData=r*cos(wedge(w2).lims(2))+centerOld(2);
                wedge(w2).line.YData=r*sin(wedge(w2).lims(2))+centerOld(1);
            else
                wedge(w).line = plot(hAx.center, ...
                    r*cos(wedge(w).lims(2))+centerOld(2), ...
                    r*sin(wedge(w).lims(2))+centerOld(1), 'k');
                wedge(w2).line = plot(hAx.center, ...
                    r*cos(wedge(w2).lims(2))+centerOld(2), ...
                    r*sin(wedge(w2).lims(2))+centerOld(1), 'k');
            end
            if (w == 1 || w == nWedges/2)
                continue
            end
            if ~isempty(wedge(w).xcorrPlt)
                ...
                hAx.xcorrPlt.XLim = wedge(w).xcorrPlt.XData([1,end]);
%                 legend(hAx.xcorrPlt,arrayfun(@(i) ...
%                     {sprintf('wedge #%i',i)},1:nWedges));
            end
            if ~isempty(wedge(w).radialProfilePlt)
                wedge(w).radialProfilePlt.YData=wedge(w).rprof;
                wedge(w2).radialProfilePlt.YData=wedge(w2).rprof;
            else
                wedge(w).radialProfilePlt = semilogy(hAx.wedge(w), ...
                    wedge(w).rprof);
                hold(hAx.wedge(w), 'on');
                grid(hAx.wedge(w), 'on');
                wedge(w2).radialProfilePlt=semilogy(hAx.wedge(w), ...
                    wedge(w2).rprof);
                legend(hAx.wedge(w), 'upper half', 'lower half')
            end
